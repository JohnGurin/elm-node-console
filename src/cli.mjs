...
import * as U from "./cliUtils.mjs";

const commaSplit = opt => opt.split(",");

const cliConfig: U.CliConfig = [
    {
        name: "mainOptions",
        opts: [
            { name: "elmBin", def: "elm", key: "b" },
            { name: "tmpDir", def: "elm-stuff", key: "t" },
            { name: "runnerModuleName", def: "NodeConsole_", key: "n" },
            {
                name: "isNodeStderr",
                key: "e",
    ...
    },
    {
        ...
        opts: [
            ...
            { name: "prefixes", key: "p", fn: commaSplit },
            { name: "suffixes", key: "s", fn: commaSplit },
            { name: "regexpes", key: "r", fn: commaSplit }
        ]
    }
];

const cliOpts = U.argsToCliOpts(gar(process.argv.slice(2)), cliConfig);
process.env.DEBUG && console.log(cliOpts);

if (
    Object.keys(cliOpts.varOptions).length === 0 ||
    !cliOpts.elmFiles ||
    cliOpts.elmFiles.length === 0
) {
    ...
    console.log(U.help(cliConfig), usage);
} else {
    mainCli(cliOpts);
}
