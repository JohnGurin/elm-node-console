...
function performComputation(file, parser) {
...
function gatherModulesData(files, varNameExaminer) {
  const modulesData = [];
  const parser = Parser(varNameExaminer);
  let sequence = Promise.resolve([]);
  files.forEach(file => {
    sequence = sequence.then(() => performComputation(file, parser)).then(result => {
      if (result) modulesData.push(result);
    });
  });
  return sequence.then(() => modulesData);
}
function varNameExaminerFactory(varOptions) {
  const {
    isAll,
    predefined,
    prefixes,
...
function prefixesExaminerFactory(prefixes) {
  return prefixes.length === 1 ? varName => varName.startsWith(prefixes[0]) : varName => prefixes.some(prefix => varName.startsWith(prefix));
}

function suffixesExaminerFactory(suffixes) {
  return suffixes.length === 1 ? varName => varName.endsWith(suffixes[0]) : varName => suffixes.some(suffix => varName.endsWith(suffix));
}
...
export { gatherModulesData, varNameExaminerFactory };
