...
function init(varNameExaminer) {
  ...
  function accumulateWord(wordTester = s => true, next, stopper = isWhiteSpace) {
    return function (s) {
      if (stopper(s)) {
        if (wordTester(word)) {
          feedSymbol = next;
  ...
  function awaitEqualSign(s) {
    ...
    if (s !== "=") {
      ...
      if (isExposing(word) && varNameExaminer(word)) vars.push(word);
      feedSymbol = awaitNewLine;
    }
  }

  const begin = awaitWord(accumulateWord(w => w === "module", awaitWord(accumulateWord(w => {
    moduleName = w;
    return true;
  ...
  function getResults() {
    if (moduleName && vars.length > 0) return {
      name: moduleName,
      vars: vars.slice()
    };
  }
